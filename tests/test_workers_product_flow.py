...
def test_product_document_task_marks_document_and_job_failed_on_index_error(
    tmp_path: Path,
    monkeypatch,
):
    Session = _session_factory()
    ...
    def fail_index(self, *, owner_id, document_id, artifact_id, require_ready=True):
        raise RuntimeError("index unavailable")

    monkeypatch.setattr(StudyDocumentIndexService, "index_artifact", fail_index)
    ...
    assert job.error_message == "index unavailable"
    assert chunks == []
