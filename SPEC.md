...
- Study Agent document evidence: `/api/study-agent/query` requires explicit `document_ids`; the default runtime loads only the authenticated user's ready documents, prefers persisted chunks from the latest `normalized_document` artifacts, falls back to query-time chunking when the persisted index is missing/incomplete/stale, and returns grounded results with citations and review metadata.
